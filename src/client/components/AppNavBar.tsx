...
const navigation = [
  { name: 'Inventory View', href: routes.ItemViewRoute.build() },
  { name: 'Receive', href: routes.FileUploadRoute.build() },
  { name: 'Build Plans', href: routes.PricingPageRoute.build() },
  { name: 'Help', href: routes.HelpRoute.build() },
...
const NavLogo = () => (
  <div className="flex items-center gap-2">
    <img className='h-8 w-8' src={logo} alt='RheoInv' />
    </div>
);;
  ...
  return (
    <header className='absolute inset-x-0 top-0 z-50 shadow sticky bg-white bg-opacity-50 backdrop-blur-lg backdrop-filter dark:border dark:border-gray-100/10 dark:bg-boxdark-2'>
      ...
      <div className='flex items-center lg:flex-1'>
          ...
          >
            <NavLogo />
            <span className='ml-2 text-sm font-semibold leading-6 dark:text-white'>RheoInv</span>
          </a>
        </div>
